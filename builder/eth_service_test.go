...
func TestBuildBlock(t *testing.T) {
	...
	err := service.BuildBlock(testPayloadAttributes, func(block *types.Block, blockValue *big.Int, _ []*types.BlobTxSidecar, _ time.Time, _, _ []types.SimulatedBundle, _ []types.UsedSBundle) {
		executableData := engine.BlockToExecutableData(block, blockValue, nil, nil)
		require.Equal(t, testPayloadAttributes.SuggestedFeeRecipient, executableData.ExecutionPayload.FeeRecipient)
