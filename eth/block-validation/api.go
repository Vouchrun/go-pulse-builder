...
// Register adds the flashbots block-validation API to the full node.
...
func (api *BlockValidationAPI) ValidateBuilderSubmissionV3(params *BuilderBlockValidationRequestV3) error {
	// PulseChain is Capella-era (Deneb disabled), so blob-bearing submissions
	// are not supported on this network.
	return errors.New("unsupported on PulseChain")
}
...
func (api *BlockValidationAPI) validateBlock(block *types.Block, msg *builderApiV1.BidTrace, registeredGasLimit uint64) error {
	...
	if api.accessVerifier != nil {
		...
		precompileSet := make(map[common.Address]struct{}, len(precompiles))
		for _, addr := range precompiles {
			precompileSet[addr] = struct{}{}
		}
		tracer = logger.NewAccessListTracer(nil, precompileSet)
