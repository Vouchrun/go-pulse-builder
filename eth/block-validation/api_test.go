...
import (
	...
	beacon "github.com/ethereum/go-ethereum/consensus/beacon"
...
func TestBlacklistLoad(t *testing.T) {
	...
	require.NoError(t, av.verifyTraces(logger.NewAccessListTracer(nil, nil)))
	...
	tracer := logger.NewAccessListTracer(acl, nil)
...
func buildBlock(args buildBlockArgs, chain *core.BlockChain) (*engine.ExecutableData, error) {
	...
	vmConfig := vm.Config{}
	blockContext := core.NewEVMBlockContext(header, chain, &args.feeRecipient)
	vmenv := vm.NewEVM(blockContext, statedb, chain.Config(), vmConfig)
	for i, tx := range args.txs {
		statedb.SetTxContext(tx.Hash(), i)
		receipt, err := core.ApplyTransaction(vmenv, &gasPool, statedb, header, tx, &header.GasUsed)
		if err != nil {
			return nil, err
		}
	...
	body := &types.Body{Transactions: args.txs, Withdrawals: args.withdrawals}
	block, err := chain.Engine().FinalizeAndAssemble(chain, header, statedb, body, receipts)
	if err != nil {
		return nil, err
	}

	execData := engine.BlockToExecutableData(block, common.Big0, nil, nil)
