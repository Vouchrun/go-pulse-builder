...
// ValidatePayload validates a builder-submitted block against the local chain
// state, including the proposer payment to the fee recipient. Ported from the
// Vouch builder fork (flashbots block-validation slice), adapted to the geth
// 1.15 core APIs (ProcessResult, ValidateState, no ForkChoice).
func (bc *BlockChain) ValidatePayload(block *types.Block, feeRecipient common.Address, expectedProfit *big.Int, registeredGasLimit uint64, vmConfig vm.Config, useBalanceDiffProfit, excludeWithdrawals bool) error {
	...
	header := block.Header()
	if err := bc.engine.VerifyHeader(bc, header, parent); err != nil {
		return err
	}

	// The block-validation endpoint validates fresh builder submissions, which
	// are not yet in the chain, so the old ForkChoice-based reorg guard never
	// fired for them (GetTd of an unknown block is nil). The geth 1.15 fork
	// choice was removed; the equivalent guard is to reject blocks that are
	...
	// any dangling prefetcher, without defering each and holding on live refs.
	defer statedb.StopPrefetcher()

	feeRecipientBalanceBefore := new(uint256.Int).Set(statedb.GetBalance(feeRecipient))

	res, err := bc.processor.Process(block, statedb, vmConfig)
	if err != nil {
		return err
	}
